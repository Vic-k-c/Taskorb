...
const { getLockedBoardIds } = require('./plans');
...
// and req.plan (set in server.js's org-context middleware).
function requireBoardAccess(minPermission) {
  return async (req, res, next) => {
    ...
    try {
      ...
      // Boards beyond the org's current plan limit are read-only (see
      // lib/plans.js#getLockedBoardIds -- oldest boards stay active,
      // newest ones lock). Always computed so a GET route can show a
      // banner even at viewer level; only enforced as a hard block for
      // anything above viewer.
      const lockedIds = await getLockedBoardIds(req.orgId, req.plan ? req.plan.boardLimit : null);
      req.boardLocked = lockedIds.has(Number(boardId));
      if (req.boardLocked && minPermission !== 'viewer') {
        return res.status(403).json({
          error: 'This board is read-only because it\'s over your current plan\'s board limit. Upgrade to make changes.',
          upgradeUrl: '/billing/upgrade?reason=boardLimit'
        });
      }
