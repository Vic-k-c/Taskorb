...
// Includes billing fields so res.locals.currentOrg (built from this) can
// carry plan context through the whole request without a second query.
async function getUserOrgs(userId, isPlatformAdmin) {
  if (isPlatformAdmin) {
    const { rows } = await pool.query(
      `SELECT id, name, slug, 'admin' AS role,
              plan_key, billing_cycle, subscription_status,
              current_period_end, grace_period_ends_at, trial_ends_at, has_used_trial
  ...
  const { rows } = await pool.query(
    `SELECT o.id, o.name, o.slug, om.role,
            o.plan_key, o.billing_cycle, o.subscription_status,
            o.current_period_end, o.grace_period_ends_at, o.trial_ends_at, o.has_used_trial
...
// Full org row (including Paystack references) for billing pages/routes --
// getUserOrgs() above deliberately leaves those out since they're never
// needed outside routes/billing.js.
...
module.exports = { slugify, uniqueSlug, getOrgRole, getUserOrgs, getOrgById, requireOrg, requireOrgApi, requireOrgRole, requirePlatformAdmin };
