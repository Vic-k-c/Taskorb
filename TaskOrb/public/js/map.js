(function () {
  ...
  // Reverted to plain OpenStreetMap tiles: CARTO's "free" Voyager endpoint
  // turned out to require an API key after all (their policy apparently
  // changed since -- confirmed live, not something I could verify from
  // here). This is the one option actually confirmed free/keyless with no
  // account risk. It's more muted than Google's style, but does still show
  // building footprints and place/POI labels once zoomed in.
  const streetLayer = L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19,
    attribution: '&copy; OpenStreetMap contributors'
  });

  // Satellite imagery alone has no street/place names -- pair it with Esri's
  // transparent reference/labels tiles on top so the hybrid view is
  // actually usable for navigating, not just pretty.
  ...
  // styling) so it looks native without adding a new stylesheet.
  let allPins = [];
  ...
  const FilterControl = L.Control.extend({
    ...
    onAdd: function () {
      ...
      const toggleBtn = L.DomUtil.create('button', 'btn ghost small', wrap);
      toggleBtn.type = 'button';
      toggleBtn.style.background = '#fff';
      toggleBtn.style.boxShadow = '0 1px 4px rgba(0,0,0,0.3)';
      toggleBtn.textContent = 'Filters';

      const panel = L.DomUtil.create('div', 'card-panel', wrap);
      panel.style.cssText = 'position:absolute; top:38px; right:0; width:220px; display:none; z-index:1000;';

      const boards = window.BOARDS_DATA || [];
      const allTagsMap = {};
      boards.forEach((b) => (b.tags || []).forEach((t) => { allTagsMap[t.id] = t; }));
      ...
      boardSel.addEventListener('change', () => {
        ...
        filterState.listId = '';
        const board = boards.find((b) => String(b.id) === boardSel.value);
        if (board) {
  ...
  function renderFilteredPins() {
    ...
    allPins.filter(pinMatchesFilter).forEach((p) => {
      ...
      const marker = L.circleMarker([p.lat, p.lng], {
        ...
        fillColor: pinColor, fillOpacity: 0.9
      });
  ...
  // --- Create-card modal: a fixed overlay, NOT a Leaflet popup anchored to
  // the map. A map-anchored popup re-positions (and can auto-pan the whole
  // map) as you interact with it, which felt like the form was "moving
  // around". This instead reuses the app's existing modal pattern (same
  // .modal-backdrop / .modal-box classes as the board's card/settings
  // modals) so it stays fixed and centered regardless of the map underneath.
