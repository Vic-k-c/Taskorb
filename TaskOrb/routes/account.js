...
router.get('/account', requireAuth, async (req, res) => {
  const orgs = await getUserOrgs(req.session.user.id);
  res.render('account', { templates: TEMPLATES, orgs, currentUser: req.session.user, error: null, success: null });
});
...
router.post('/account', requireAuth, async (req, res) => {
  ...
  const render = async (error, success) => {
    const orgs = await getUserOrgs(req.session.user.id);
    res.render('account', { templates: TEMPLATES, orgs, currentUser: req.session.user, error, success });
  };
  ...
  try {
    ...
    const { rows: updated } = await pool.query(
      `UPDATE users SET name = $1, email = $2, password_hash = $3, default_template = $4
       WHERE id = $5 RETURNING id, name, email, default_template, is_platform_admin`,
