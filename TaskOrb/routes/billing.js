...
const { planList, getPlan, getOrgUsage, applyChargeSuccess, startTrial, TRIAL_DAYS } = require('../lib/plans');
...
router.get('/billing', requireAuth, requireOrg, requireOrgRole('admin'), async (req, res) => {
  ...
  res.render('billing', { org, plan, usage, transactions, trialDays: TRIAL_DAYS, checkoutStatus: req.query.checkout || null, currentUser: req.session.user });
});
...
router.get('/billing/upgrade', requireAuth, requireOrg, async (req, res) => {
  ...
  res.render('pricing', {
    ...
    hasUsedTrial: org.has_used_trial,
    isOnTrial: org.subscription_status === 'trial',
...
// --- Start a free trial. Admin-only, no payment involved. One per org,
// ever -- see lib/plans.js#startTrial. ---
router.post('/billing/start-trial', requireAuth, requireOrg, requireOrgRole('admin'), async (req, res) => {
  const { planKey } = req.body;
  try {
    await startTrial(req.orgId, planKey);
    res.redirect('/billing?checkout=trial-started');
  } catch (err) {
    res.redirect(`/billing/upgrade?error=${encodeURIComponent(err.message)}`);
  }
});
