...
const { getLockedBoardIds, requireAttachmentRoom } = require('../lib/plans');
...
// Shared by every editor-level check below: is this board past the org's
// current plan limit? See lib/plans.js#getLockedBoardIds.
async function isBoardOverLimit(req, boardId) {
  const lockedIds = await getLockedBoardIds(req.orgId, req.plan ? req.plan.boardLimit : null);
  return lockedIds.has(Number(boardId));
}
...
async function requireCardEditor(req, res, next) {
  try {
    ...
    if (await isBoardOverLimit(req, boardId)) {
      return res.status(403).json({ error: 'This board is read-only because it\'s over your current plan\'s board limit.', upgradeUrl: '/billing/upgrade?reason=boardLimit' });
    }
...
router.post('/api/cards/:id/attachments', requireAuth, requireOrgApi, requireCardEditor, upload.array('files', 5), requireAttachmentRoom, async (req, res) => {
  try {
