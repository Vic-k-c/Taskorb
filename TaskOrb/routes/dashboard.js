...
router.get('/', requireAuth, requireOrg, async (req, res) => {
  ...
  const { rows: byTag } = await pool.query(
    `SELECT t.name, t.color, COUNT(DISTINCT ct.card_id)::int AS count
     FROM tags t
     LEFT JOIN card_tags ct ON ct.tag_id = t.id
     LEFT JOIN cards c ON c.id = ct.card_id
     LEFT JOIN lists l ON l.id = c.list_id
     LEFT JOIN boards b ON b.id = l.board_id AND b.org_id = t.org_id
     ${isOrgAdmin ? '' : 'LEFT JOIN board_members bm ON bm.board_id = b.id AND bm.user_id = $2'}
     WHERE t.org_id = $1 AND (ct.card_id IS NULL ${isOrgAdmin ? '' : 'OR bm.user_id IS NOT NULL'})
