...
router.get('/export/pdf', requireAuth, requireOrg, requireFeature('hasExport'), async (req, res) => {
  try {
...
router.get('/export/csv', requireAuth, requireOrg, requireFeature('hasExport'), async (req, res) => {
  try {
    const rows = await getExportRows(req.session.user.id, req.orgId, req.orgRole);
