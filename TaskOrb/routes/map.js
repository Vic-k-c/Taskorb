...
const { requireFeature, requireFeatureApi } = require('../lib/plans');
...
router.get('/map', requireAuth, requireOrg, requireFeature('hasMap'), async (req, res) => {
...
router.post('/api/map-pins', requireAuth, requireOrgApi, requireFeatureApi('hasMap'), async (req, res) => {
...
// Best-effort reverse geocoding proxy (Google Geocoding API) so the popup
// form can prefill an address from the tapped coordinates. Uses the same
// key as the map itself unless GOOGLE_GEOCODING_API_KEY is set separately
// (useful if you want a server-only key restricted by IP instead of one
// that's also embedded in the page HTML).
router.get('/api/reverse-geocode', requireAuth, async (req, res) => {
  ...
  if (!apiKey) return res.json({ address: '' });
  try {
    ...
    if (!resp.ok) {
      ...
      console.error(`Reverse geocode failed: Google returned ${resp.status}. Body preview: ${bodyPreview}`);
      return res.json({ address: '' });
    }
    const data = await resp.json();
    if (data.status && data.status !== 'OK' && data.status !== 'ZERO_RESULTS') {
      console.error(`Reverse geocode failed: Google status ${data.status} - ${data.error_message || ''}`);
    }
    const address = (data.results && data.results[0] && data.results[0].formatted_address) || '';
    res.json({ address });
  } catch (err) {
