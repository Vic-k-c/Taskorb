...
const { seedDefaultTags } = require('../lib/tags');
...
router.post('/orgs', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const { rows: orgRows } = await client.query('INSERT INTO organizations (name, slug) VALUES ($1, $2) RETURNING id', [name.trim(), slug]);
    const orgId = orgRows[0].id;
    await client.query("INSERT INTO org_members (org_id, user_id, role) VALUES ($1, $2, 'admin')", [orgId, req.session.user.id]);
    await seedDefaultTags(client, orgId);
