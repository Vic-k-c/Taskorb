...
const { requireSeatLimit } = require('../lib/plans');
...
router.post('/users', requireAuth, requireOrg, requireOrgRole('admin'), requireSeatLimit, async (req, res) => {
  ...
  if (role === 'admin' && !req.plan.hasMultiAdmin) {
    const { rows: adminCount } = await pool.query(
      "SELECT COUNT(*)::int AS count FROM org_members WHERE org_id = $1 AND role = 'admin'",
      [req.orgId]
    );
    if (adminCount[0].count >= 1) {
      return render(`Your ${req.plan.name} plan only supports one org admin. Upgrade to Team Growth or higher for multiple admins.`);
    }
  }
