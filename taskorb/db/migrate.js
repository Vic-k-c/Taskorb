...
async function migrate() {
  ...
  if (!(await columnExists('users', 'default_template'))) {
    console.log('Adding users.default_template...');
    await pool.query("ALTER TABLE users ADD COLUMN default_template TEXT DEFAULT 'blank'");
  }

  // Platform-level super-admin (separate from any org's own admins), for
  // running this as a SaaS. Bootstrapped via an env var rather than any
  // in-app action, so nobody can grant it to themselves through the UI.
  ...
  if (!(await columnExists('boards', 'org_id'))) {
    console.log('Adding boards.org_id...');
    await pool.query('ALTER TABLE boards ADD COLUMN org_id INT REFERENCES organizations(id) ON DELETE CASCADE');
    await pool.query('CREATE INDEX IF NOT EXISTS idx_boards_org_id ON boards(org_id)');
  }
  if (!(await columnExists('notifications', 'org_id'))) {
    console.log('Adding notifications.org_id...');
    await pool.query('ALTER TABLE notifications ADD COLUMN org_id INT REFERENCES organizations(id) ON DELETE CASCADE');
  }
  ...
  const { rows: orglessBoards } = await pool.query('SELECT id, owner_id FROM boards WHERE org_id IS NULL');
  if (orglessBoards.length > 0) {
    console.log(`Found ${orglessBoards.length} board(s) with no organization -- creating a legacy org...`);
    const { rows: legacyOrgRows } = await pool.query('SELECT id FROM organizations WHERE slug = $1', ['legacy']);
    let legacyOrgId;
    if (legacyOrgRows[0]) {
    ...
    // Every pre-existing user becomes a member of the legacy org, keeping
    // whatever global role they had before roles became per-org.
    const { rows: allUsers } = await pool.query('SELECT id, role FROM users');
    ...
    await pool.query('UPDATE boards SET org_id = $1 WHERE org_id IS NULL', [legacyOrgId]);
    await pool.query('UPDATE notifications SET org_id = $1 WHERE org_id IS NULL', [legacyOrgId]);
