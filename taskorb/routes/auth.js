...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    req.session.user = { id: user.id, name: user.name, email: user.email, role: user.role, default_template: user.default_template };
    res.redirect('/');
  } catch (err) {
    console.error(err);
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const { rows: inserted } = await pool.query(
      'INSERT INTO users (name, email, password_hash, role) VALUES ($1, $2, $3, $4) RETURNING id, name, email, role, default_template',
